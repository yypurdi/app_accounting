...
router.get('/', sessionChecker, function(req, res, next) {
    ...
    jsonObj.rowid = 0;
    req.session.previouspath = 'readAppEmploymentStatus?'+req._parsedUrl.query;
    parameter = {
        session: req.session,
        cookies: req.cookies,
    ...
    var readAppEmploymentStatus = new Promise((resolve, reject) => {
        setTimeout(() => {
            dao.readAppEmploymentStatus(jsonObj,function(err,rows) {
                if(err) res.status(err.code).end();
                else{
                    if(rows[0]===undefined) res.redirect('/searchAppEmploymentStatus');
                    else{ 
                        jsonObj = rows[0];
                        resolve(jsonObj);
                    }
    ...
    var sendResult = (results) => {
        ...
        res.render('AppEmploymentStatusReadPage',parameter);
    }    
    Promise.all([readAppEmploymentStatus,appEmployee]).then(sendResult);
});
